...
SEXP eiIndiMDG1(SEXP C, SEXP R, SEXP AGGPREC,
...
////////// Einlesen und Umwandeln aller Vektoren und Werte
				...
				if((betanew>0) & (betanew<(betarefcurr+betacurr))){
					...
					for(int qq=0;qq<r;qq++){
						betasumcurr +=betas[r*c*pp + qq + r*cc] * xdf[qq + (aggp-1)*qq + pp];
						 // aequivalent  xdf[pp + (aggp*qq)]
						 // R indizes bspl ((0:2)+20*(0:2)+0)+1 --- 1 22 43
					...
					// Rprintf("alpharc %f -- alphaRC %f", alphas[rr+r*cc],alphas[rr+r*(c-1)]);
					// Rprintf("\n");
	...
	for(rr=0; rr<r; rr++){
		for(cc=0; cc<c; cc++){
			...
			//Rprintf("%f %f ~ %f\n", alphasumnew, alphasumcurr, alphasumcurr - alphacurr);
			
			if(alphanew>0){
				llalphacurr = llAlphaGamma(aggp, alphasumcurr, alphacurr, logbetasum[rr+cc*r], la1,la2);
				llalphanew  = llAlphaGamma(aggp, alphasumnew, alphanew, logbetasum[rr+cc*r], la1,la2);
				// Rprintf("%f\n",llalphanew);
				u=log(runif(0,1));
				// Rprintf("%f -- ",alphacurr);
