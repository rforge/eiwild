...
SEXP eiIndiMDE1(SEXP C, SEXP R, SEXP AGGPREC,
...
for(ii=0; ii<iter;ii++){
	...
	for(rr=0; rr<r; rr++){
		for(cc=0; cc<c; cc++){
			...
			if(alphanew>0){
				llalphacurr = llAlphaExpo(aggp, alphasumcurr, alphacurr, logbetasum[rr+cc*r], la);
				llalphanew  = llAlphaExpo(aggp, alphasumnew, alphanew, logbetasum[rr+cc*r], la);
				// Rprintf("%f\n",llalphanew);
				u=log(runif(0,1));
				// Rprintf("%f -- ",alphacurr);
